...
`PacketAnalysis` is a frozen dataclass retaining the exact original `observation` and the exact decoder-produced model references in `ethernet`, `ipv4`, `tcp`, `udp`, `icmp`, and `ipv6`. The optional `ipv6` field is appended after the existing fields to preserve positional construction. The IPv6 path calls only the base-header decoder and returns `ethernet` and `ipv6`; direct construction rejects combining IPv6 with IPv4, decoded transport models, or checksum results. Unused optional model fields are `None`. Its four optional boolean fields are `ipv4_checksum_valid`, `tcp_checksum_valid`, `udp_checksum_valid`, and `icmp_checksum_valid`. Direct construction checks model types and requires checksum results to be booleans or `None`.
