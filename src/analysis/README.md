...
Admission means that every applicable accumulator successfully accepts the same exact `PacketAnalysis` object. Successful packet analysis or `FlowTracker.record()` alone does not establish admission here. The fixed protocol-neutral participants are `FlowStatistics`, `DirectionalFlowStatistics`, `FlowPacketSizeStatistics`, `FlowInterArrivalStatistics`, and `DirectionalInterArrivalStatistics`. TCP flows additionally include `TCPControlStatistics`, optional `LDAPFlowStatistics`, and `TCPStreamState`; UDP flows carry explicit `None` in those fields. The first two protocol-neutral components provide volume and temporal extent inputs, packet-size statistics retain length moments, and the last two retain adjacent global and same-direction timing. Directional timing participates as complete raw state and supplies its directional feature family; it cannot be reconstructed from global aggregates.
...
The fixed bound is **65536 bytes per direction per window**, enough to expose the current LDAP parser's maximum candidate input. Concatenation checks this limit before allocation; overlap comparisons cover at most retained bytes. State holds no segment queue or predecessor chain. At capacity, duplicates and empty observations remain possible, but further contiguous data makes the prefix unavailable. This is a prefix limit, not a sliding buffer or consumption API. Active-flow counts and caller-retained historical windows retain the repository's existing ownership/resource limits; there is no new global tracker or quota system.

For known plaintext LDAP starting at the observed prefix, an explicit consumer can use:

```python
stream = window.coordinated_state.tcp_stream_state.forward
payload = None if stream is None else stream.contiguous_payload
ldap = None if not payload else analyze_ldap_payload(payload)
```

The existing parser yields the same framing for a whole message and the same bytes split across contiguous segments. Retransmissions do not duplicate bytes. A gap leaves only the earlier incomplete prefix and unavailable continuation. This does not automatically reparse streams, revise LDAP counters, decrypt TLS/LDAPS, or implement an incremental protocol consumer. Buffer consumption and incremental framing could be future work for LDAP and other stream protocols. Packet-loss recovery, out-of-order queues, general TCP reassembly, SACK, congestion control, timers, TCP state-machine reconstruction, and attack detection remain outside scope.
