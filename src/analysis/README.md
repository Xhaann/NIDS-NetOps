...
Admission means that every participating accumulator successfully accepts the same exact `PacketAnalysis` object. Successful packet analysis or `FlowTracker.record()` alone does not establish admission here. The fixed participants are `FlowStatistics`, `DirectionalFlowStatistics`, `FlowPacketSizeStatistics`, `FlowInterArrivalStatistics`, and `DirectionalInterArrivalStatistics`. The first two provide volume and temporal extent inputs, packet-size statistics retain length moments, and the last two retain adjacent global and same-direction timing. Directional timing participates as complete raw state and supplies its directional feature family; it cannot be reconstructed from global aggregates.
...
Candidates are constructed in the participant order above using the previous published components and the same packet and canonical identity. Only after all five updates and the result construction succeed does one reference assignment publish the new state. Any exception propagates unchanged and leaves the previous state, every previous component, and packet inputs unchanged. Earlier successful candidates are discarded; no rollback, partial publication, automatic retry, or rejected-packet retention occurs. This is a synchronous contract for sequential, non-overlapping calls; it does not provide concurrent-writer or durable transaction guarantees.

The frozen `CoordinatedFlowState` has exactly five fields: `flow_statistics`, `directional_flow_statistics`, `flow_packet_size_statistics`, `flow_inter_arrival_statistics`, and `directional_inter_arrival_statistics`. These retain exact updater-produced objects. Its read-only `identity` property returns `flow_statistics.identity`, without storing another identity or copying any numerical fields. Direct construction checks exact component types and equal flow identities; wrong types raise `TypeError`, and incompatible identities raise `FlowCoordinationError`.

The provenance guarantee belongs to the coordinator's construction protocol: every state it publishes represents the same ordered sequence of successfully admitted analysis objects in all five components. By induction, the empty owner starts all components together, each successful call advances all components with one common packet, and a failed call advances none. Repeated successful calls with the same packet count separately; there is no deduplication. Neither numerical equality, object equality, nor direct construction of a `CoordinatedFlowState` proves this history. Manually assembled or replaced bundles cannot be imported into a coordinator through its public API. References identify the retained sources but are not persistent provenance identifiers, and the packet sequence cannot be recovered from these aggregates.
...
Consumers can retain one published state and use its exact components with existing extractors. This provides coherent source inputs without binding arbitrary precomputed numerical features to a flow. The [feature snapshot boundary](flow_feature_snapshot.py) composes those extractors from a coordinator's current publication. The coordinator holds one current state, retains no analyses, observations, packet history, or timestamp history, and has a fixed number of retained objects regardless of packet count. `FlowTracker` remains independent and retains its existing tracking API and semantics.

## Typed flow feature snapshots

[flow_feature_snapshot.py](flow_feature_snapshot.py) exports `FlowFeatureSnapshot`, `FlowFeatureSnapshotError(ValueError)`, and `extract_flow_feature_snapshot(coordinator: FlowStateCoordinator) -> FlowFeatureSnapshot`. The function requires exactly `FlowStateCoordinator`, reads its current publication once, and requires exactly `CoordinatedFlowState`. An empty coordinator raises `FlowFeatureSnapshotError`. Wrong types, subclasses, mocks, bare states, and caller-supplied feature combinations are rejected with `TypeError`. Direct snapshot construction and `dataclasses.replace()` are not supported; extraction is the sole public construction path.

The coordinator argument is deliberate: a bare `CoordinatedFlowState` can be assembled independently, so its type alone does not prove coordinated admission. Obtaining the state through the unchanged coordinator's publication API establishes the existing construction-protocol guarantee without identifiers or registries. The guarantee assumes use of the public APIs, not private-state tampering. It is an in-process source association, not a persistent identifier, recoverable packet history, or proof inferred from numerical or object equality. Calls follow the coordinator's sequential, non-overlapping use contract.

The frozen snapshot retains exactly these seven typed fields in the listed order:

| Field | Source and extraction path |
| --- | --- |
| `coordinated_state` | Exact publication read from the coordinator. |
...
The snapshot retains the exact objects returned by each extractor and exposes `identity` through `coordinated_state.identity`. It stores no duplicate identity, raw source fields, raw directional statistics, intermediate `FlowFeatureInput`, coordinator reference, cache, or flattened values. The directional feature object is derived from the raw directional statistics already present in the retained publication. Its forward and reverse groups independently contain five `None` values when that direction has no intervals; an observed zero-second interval produces five real `0.0` values. Global features continue to describe adjacent packets, while directional features describe same-direction intervals. Later admissions do not change an existing snapshot, and repeated extraction from an unchanged publication produces equal values without caching.

One-packet flows preserve zero duration, absent rates, zero global inter-arrival features, and unavailable directional inter-arrival groups. Equal packet timestamps and unused directions preserve their established interval and packet-size semantics. Mathematical definitions, population variances, rounding policy, and validation remain owned by the existing extractors. Eager typed composition avoids repeated lazy extraction and duplicate raw-state references. This is not an ML vector or detector input contract; no numerical ordering, flattening, serialization, detection, or machine learning is defined.
