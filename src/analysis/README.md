...
`decode_ipv6()` remains a base-header-only operation. `analyze_packet()` invokes validation and exposes the completed chain in `PacketAnalysis.ipv6_extension_headers`, retaining the exact IPv6 packet context. This feature includes no fragment semantics or reassembly, ICMPv6, Neighbor Discovery, transport decoding, flow integration, feature extraction, detectors, or security interpretation.
