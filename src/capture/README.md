...
An `OSError` from obtaining or advancing the iterator becomes `CaptureError` with the original exception chained as its cause. Existing `CaptureError` and other exceptions propagate unchanged. Invalid record types raise `TypeError`, consistent with `PacketObservation`; the adapter stops delivery instead of skipping the record. Interruptions during startup or acquisition and exceptions during timestamp/observation construction also mark the session failed before propagating. Only `StopIteration` from advancing the producer means normal exhaustion; if observation construction raises it, the adapter raises `CaptureError` with that exception as its cause. No subsequent record is read through a failed source before cleanup. This adapter adds no file format or packet-capture dependency.
