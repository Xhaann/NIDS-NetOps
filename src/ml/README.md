...
The separate [ResearchExample](../research/README.md) contract can retain a projection alongside optional caller-supplied research truth text. Truth never becomes a projection column, and projection does not import or invoke the research package. Research examples do not reinterpret detector/evaluation truth or construct datasets.
...
The deterministic pipeline does not import or invoke `ml`. Projection is an explicit separate call and changes no detection, finding, truth, evaluation, metrics, benchmark, reporting, capture, or CLI semantics. Models, preprocessing, ML datasets, training, inference, model selection and MLOps remain deferred. No ML accuracy claim is made.
