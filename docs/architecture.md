...
The application [performance benchmark](../src/application/performance_benchmark.py) measures an explicit synchronous callable through configured warmups and measured repetitions. Frozen `PerformanceBenchmarkConfiguration` declares the operation ID/version, counts, and optional existing dataset/experiment/system-configuration references. Frozen `PerformanceBenchmarkResult` retains that configuration and ordered float elapsed seconds, with minimum, maximum, accurate total, mean, and median projections. No operation result or mutable callable is stored in the timing value object.

The default clock is `time.perf_counter`; deterministic tests supply finite nondecreasing float readings. Warmups are untimed and excluded from statistics. Each measured execution receives exactly two clock reads, and failures propagate without retry or a partial success result. Methodology is deterministic, while real elapsed observations remain environment-sensitive. No machine-speed assertions, throughput inference, overhead subtraction, or profiling is introduced.
