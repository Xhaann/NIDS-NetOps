...
Successful packet analysis includes base-header decoding and structural traversal of the supported extension headers. TCP, UDP, and the common ICMPv6 header are decoded at their safe terminal boundaries; unsupported upper-layer content remains opaque. Analysis invokes no detector, flow admission, or lifecycle operation. Existing IPv4-only flow and threshold boundaries remain intact. Fragment Header semantics are analyzed in the packet-local boundary below. Reassembly, ICMPv6 subtype decoding, Neighbor Discovery, IPv6 transport checksum validation, and IPv6 detection remain outside this feature.
