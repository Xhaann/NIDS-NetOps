...
| IPv6 extension-header statistics | Admitted IPv6 TCP/UDP packets contribute directional packet counts, chain cardinality, sparse 8-bit extension and terminal Next Header distributions, and repeated-type counts. The immutable aggregate bounds each chain at 8,191 entries and retains no packet or payload history. |
...
`PacketAnalysis.dns` exposes a lazy, immutable DNS observation from the established IPv4/IPv6 UDP payload boundary on source or destination port 53. The port-independent `analyze_dns_message()` also accepts an already-delimited message supplied by a caller. Feature 22 frames TCP DNS at the existing flow-owned stream boundary; packet-local `PacketAnalysis.dns` remains UDP-only. DNS observation does not participate in detection or change packet validity, flow admission, publication, evaluation, or metrics.

Header, ordered question/record envelopes and binary names are parsed with explicit message, entry, expanded-name and pointer limits. Ordinary malformed/truncated input produces a status and complete parsed prefix; unsupported encodings, operational limits and pointers into opaque RDATA are explicit stops. Raw RDATA remains preserved. Unknown types stay opaque; OPT additionally receives bounded EDNS option-envelope analysis. This establishes structural observations for protocol consumers without inferring attacks. See the [DNS contract](../src/analysis/README.md#dns-message-observation) for exact limits and compression behavior. Parser state is bounded per message; caller-retained outputs remain caller-owned. No persistent archival or parallel capture/flow lifecycle is introduced.
...
The additive coordinator field and window property use the same atomic preparation/publication and newly terminal closure suffix as the previous DNS statistics. Failed aggregation leaves the prior state retryable, publication retry cannot duplicate records, and existing eviction/readmission and source release remain authoritative. The result retains no packet, message, transaction, resource record, RDATA, name or flow. IPv4/IPv6 share the established UDP path; already-delimited TCP observations can use the reducer, while automatic framing uses the bounded Feature 22 path below. Invalid DNS contributes no records, including valid prefixes of invalid messages. See the [resource-record contract](../src/analysis/README.md#dns-resource-record-structural-statistics). Generic version 1, the 49-value ML projection, LDAP, detectors and evaluation are unchanged. This layer does not detect attacks.
...
The existing record decoder first checks the full RDLENGTH envelope. The EDNS helper then reads only that bounded RDATA. Missing declared record bytes are INCOMPLETE; option framing that exceeds complete RDATA is MALFORMED. Non-root OPT owners, OPT outside additionals and duplicate OPT records are MALFORMED. Only the existing completed record prefix is retained on failure, with no semantic EDNS attached. No partial option tuple is published. Message/entry/option-count limits use UNSUPPORTED with `limit_reached=True`; infrastructure failures propagate.
...
IPv4/IPv6 UDP and already-delimited TCP observations share this parser. Automatic framing uses the bounded Feature 22 path below. Features 14–19 statistics and header semantics, generic version 1, the 49-value ML projection, LDAP, detection and evaluation remain unchanged. Previously opaque invalid OPT data now receives structural failure status; valid OPT remains one additional record for existing statistics. EDNS analysis does not detect attacks or assign security meaning.
...
TCP packet → existing `TCPStreamState` → `update_dns_stream_state` → transient complete-frame bytes → existing `analyze_dns_message` → existing correlation and DNS statistics. The framer reads only the two-byte network-order length and exact payload boundary. It neither parses DNS fields nor implements sequence tracking or reassembly. Forward/reverse state belongs to the existing flow; port 53 selects DNS framing, with existing LDAP precedence for a flow also using port 389.

`DNSStreamUpdate` separates its transient `forward_frames` / `reverse_frames` from its retained `DNSStreamState`. The state holds the exact consumed TCP state and directional `DNSStreamObservation` values, each containing only the shared TCP observation, partial prefix, declared length, incomplete payload, framing status and existing TCP failure reason. The coordinator stores only the state. `FlowObservationWindow.dns_stream_state` exposes it; no generic feature fields are added.

Each direction consumes newly observed contiguous bytes immediately, allowing TCP's existing buffer reclamation while a DNS frame spans observations. Declared message length is at most `DNS_MAX_MESSAGE_BYTES=65535`; a maximum-length frame including its prefix occupies 65,537 wire bytes and must span observations. No more than one prefix byte or 65,534 incomplete payload bytes is retained per direction outside the existing 65,536-byte TCP buffer. Complete-frame batches are bounded transient outputs, not accumulated history.

Framing READY / INCOMPLETE / UNAVAILABLE is distinct from `DNSMessageStatus`. A zero-length frame emits empty bytes to the parser, which reports INCOMPLETE; it does not block later frames. Fully framed malformed/unsupported DNS or EDNS follows existing parser/correlation admission. Partial frames never reach the parser. TCP gaps, conflicts and other unavailable conditions remain sticky using `TCPStreamStatus`, without recovery or resynchronization. A defensive over-limit declaration guard uses LIMIT_EXCEEDED before consuming body bytes; no declaration above 65,535 can currently be represented by the two-byte prefix.

Messages completing in one observation use that observation's capture timestamp and enter correlation in directional wire order. Each newly terminal correlation batch is reduced before the next message; retained correlation remains the existing latest-call batch plus bounded pending requests. Infrastructure exceptions abort the candidate using existing prepare/publication atomicity, preserving previously published state for supported retries. Ordinary DNS parse failures consume their known frame boundary and allow following frames. No DNS-specific rollback exists.

Window closure retains incomplete framing metadata without turning it into DNS, and finalizes only already-admitted DNS requests. FIN/RST can deliver a final complete frame while leaving a partial suffix; they do not independently close the flow window. Inactivity, capacity, explicit segmentation and capture end keep their existing ownership rules. New windows start framing at their newly observed stream origin, whose application-message alignment is not guaranteed for midstream capture. See the [complete framing contract](../src/analysis/README.md#dns-over-tcp-message-framing). Features 12–21 semantics, LDAP, generic version 1, all 49 projected values, detection, evaluation, metrics, capture and CLI remain unchanged.


## TLS record framing

Feature 23 adds a separate protocol-specific consumer at the existing TCP observation boundary: TCP packets → bounded directional `TCPStreamState` → `update_tls_record_state` → transient complete `TLSRecordObservation` tuples. Automatic eligibility requires port 443 and excludes flows involving port 389 or 53, preserving established LDAP and DNS precedence. Other ports are not inspected speculatively. TCP, DNS and LDAP implementations remain unchanged.

The five-byte header stores the one-byte content type, two opaque protocol-version bytes and unsigned network-order payload length. The explicit payload ceiling is 18,432 bytes. Headers and payloads may span observations, and coalesced records retain directional wire order. The framer does not interpret payloads or validate TLS semantics. There is no handshake/certificate parsing, decryption, fingerprinting or attack detection.

`CoordinatedFlowState.tls_record_state` and the corresponding window property share the exact consumed TCP state. Each direction retains only its current incomplete header or bounded incomplete payload; completed records belong to the transient update, with no retained record history. Their shared stream reference identifies the completing TCP observation. TCP has no capture-time field; callers use the existing packet/flow timestamp context without a new clock or timestamp representation.

TCP failures remain authoritative and sticky. An oversized TLS declaration stops before body consumption with UNAVAILABLE / LIMIT_EXCEEDED. Infrastructure exceptions leave candidate consumption unpublished under existing prepare/publication rules. Complete records can be consumed by a future analyzer during preparation; no TLS semantic consumer or external delivery API is introduced here. Closure preserves incomplete framing and does not fabricate records. No gap recovery or resynchronization is added, and a midstream origin cannot establish TLS alignment. See the [public contract and exact bounds](../src/analysis/README.md#tls-record-framing). Features 12–22, generic version 1, the 49-value projection, detection, evaluation, metrics, capture and CLI retain their semantics.


## TLS handshake-message framing

Feature 24 connects the existing transient `TLSRecordUpdate` to `update_tls_handshake_state` during coordinator preparation. The path is TCP packets → existing bounded directional TCP observation → existing TLS record framing → bounded handshake-message framing. Only completed records with ContentType 22 supply bytes. Other content types and empty records preserve incomplete handshake bytes without adding payload or inferring TLS session semantics.

The handshake header is four bytes: one opaque type byte and a three-byte unsigned network-order body length. Messages may span records, and one record can finish one message and begin or complete further messages. The explicit body ceiling is 262,144 bytes, below the 16,777,215-byte wire-field maximum. Oversized declarations stop handshake extraction before body copying with UNAVAILABLE / LIMIT_EXCEEDED. Existing lower layers have already consumed the bounded complete TLS record; they are not rewound or reimplemented.

`CoordinatedFlowState.tls_handshake_state` and the window property share the exact TLS record state. Per direction, handshake state contains only up to three incomplete header bytes or up to 262,143 incomplete body bytes and decoded header metadata. Complete messages belong only to transient update tuples and reference their completing record's existing TCP observation. No complete record/message, packet graph or predecessor state is retained. TCP and TLS record ownership, eligibility and failure semantics remain authoritative, including port 443 and LDAP/DNS precedence.

Infrastructure failures abort preparation/publication atomically; retry from published state reproduces the batch. Normal framing-limit failures preserve earlier completed messages and remain sticky. Closure retains incomplete state without emitting it, while new windows start independently. No semantic TLS parsing, decryption, detection, alignment scanning or resynchronization is added. Midstream alignment remains unproven. See the [handshake contract](../src/analysis/README.md#tls-handshake-message-framing) for public fields, bounds and synthetic validation limitations. Features 12–23, generic feature version 1 and all 49 projected values remain unchanged.


## TLS handshake-message structural statistics

Feature 25 consumes only the newly completed `TLSHandshakeObservation` batches from Feature 24 during coordinator preparation. The path is TCP observation → TLS record framing → TLS handshake framing → structural statistics. No TCP/TLS bytes or handshake headers are reparsed. A complete observation must have its declared body length equal to its exact payload length; inconsistent observations fail validation instead of being repaired.

`TLSHandshakeStatistics` stores message count, zero-length count, body-length minimum/maximum/total and a fixed 256-bin tuple indexed by the opaque type byte. Its mean is an exact `Fraction`, absent for empty input. `DirectionalTLSHandshakeStatistics` holds separate forward/reverse aggregates using the existing observation direction, with no combined counter or new direction model. `CoordinatedFlowState.tls_handshake_statistics` defaults to the empty pair; the window property exposes it.

Each successful incremental emission contributes once. Closure preserves already accumulated statistics; partial/unavailable framing contributes nothing, and repeated finalization cannot recount messages. Validation, aggregation, construction and publication failures leave the entire candidate unpublished for retry. New windows own independent aggregates. No new finalizer, timestamp, deduplication history or lifecycle is introduced.

The retained pair contains ten scalar values and 512 fixed distribution counters, plus its fixed container structure. Counter integer widths may grow with cumulative totals. There are no payload, record, stream, packet, message or predecessor references in the statistics. Existing framing/window retention remains separately bounded. See the [public statistics contract](../src/analysis/README.md#tls-handshake-message-structural-statistics). Features 12–24, generic version 1 and all 49 projected values retain their semantics; no TLS semantic parsing, fingerprinting, detection or ML is added.

## TLS ClientHello structural analysis

Feature 26 extends coordinator preparation after TLS handshake framing and structural statistics: newly completed type 1 handshakes → `analyze_tls_client_hello` → immutable ClientHello observations. TCP reconstruction, TLS record/handshake framing and eligibility remain unchanged. The body parser preserves version/random/session bytes, ordered cipher/compression values and ordered extensions; only supported groups, signature algorithms and ALPN receive bounded structural decoding. Duplicate occurrences remain separate and unknown extensions remain opaque.

The existing 262144-byte handshake ceiling is the outer limit. One-/two-byte vector bounds and a 1024-extension ceiling bound every representation. Lengths must fit before extraction; nested vectors fill their containers exactly, and trailing bytes are rejected. COMPLETE, INCOMPLETE, MALFORMED and UNSUPPORTED distinguish structural outcomes. Complete framing does not establish semantic TLS validity.

The coordinator and window expose `tls_client_hellos`, the current packet's result tuple. It replaces the prior batch rather than accumulating history. Results retain the existing completing stream observation and direction, without a new clock or inferred role. All parsing occurs before publication; infrastructure failures leave previous framing, statistics and analyses intact for retry. Closure returns the existing last batch without re-emission. The existing session API exposes closed snapshots, not an archive of earlier results. See the [exact parser/publication contract](../src/analysis/README.md#tls-clienthello-structural-analysis).

No TLS fingerprints, JA3/JA4, cryptographic validation, decryption, detection, ML or generic feature changes are introduced. Synthetic capture/session tests cover four classic PCAP encodings and deterministic IPv4/IPv6 bidirectional replay; no external-corpus validation is claimed.
