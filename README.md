...
Canonical `FlowIdentity` values support same-family IPv4 and IPv6 packed endpoints, with validated textual construction through the standard library. Ethernet IPv6 frames now support fixed base-header analysis with declared-payload completeness checks and opaque payload retention. Separate immutable values can represent supplied IPv6 extension-header metadata and ordered chains tied to an exact packet. Extension-header parsing and validation, IPv6 transport decoding, fragmentation, ICMPv6, and IPv6 detection remain unimplemented. Packet-derived flow admission and flow-threshold detection remain IPv4-only.
