...
Canonical `FlowIdentity` values support same-family IPv4 and IPv6 packed endpoints, with validated textual construction through the standard library. Packet parsing, packet-derived flow admission, and flow detection remain IPv4-only; IPv6 parsing and detection are not implemented.
