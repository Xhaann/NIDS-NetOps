...
- **TCP streams:** independent directional prefixes retain up to 64 KiB, append contiguous payload, suppress identical retransmissions, and expose gaps/overlaps as unavailable. See [stream contract](src/analysis/README.md#bounded-directional-tcp-stream-observation).
- **LDAP:** bounded plaintext LDAP message framing, operation identification, and immutable flow observation counters for TCP port 389. Incomplete and malformed candidates remain observations, with no LDAP attack detector. Explicit consumers can parse contiguous stream prefixes with the same parser. See [LDAP scope](src/analysis/README.md#ldap-protocol-observations).
