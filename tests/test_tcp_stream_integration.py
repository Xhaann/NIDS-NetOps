...
class TCPStreamIntegrationTests(unittest.TestCase):
    ...
    def test_pcap_streams_detection_evaluation_and_research_projection_remain_compatible(self):
        ...
        for window in windows:
            legacy = replace(window, coordinated_state=replace(window.coordinated_state, tcp_stream_state=None, tls_record_state=None, tls_handshake_state=None))
        ...
        for finding, reference in zip(actual.pipeline_result.flow_findings, previous.pipeline_result.flow_findings):
            ...
            stripped = replace(window, coordinated_state=replace(window.coordinated_state, tcp_stream_state=None, tls_record_state=None, tls_handshake_state=None))
    ...
    def test_coordinated_state_validates_extension_and_preserves_legacy_constructors(self):
        ...
        arguments = tuple(getattr(state, field.name) for field in fields(state)
                          if field.name not in ('tcp_stream_state', 'dns_transaction_statistics', 'dns_query_name_statistics', 'dns_resource_record_statistics', 'dns_message_flag_statistics', 'dns_edns_statistics', 'dns_stream_state', 'tls_record_state', 'tls_handshake_state', 'tls_handshake_statistics', 'tls_client_hellos', 'tls_client_hello_statistics'))
