...
class FlowStateCoordinatorTests(unittest.TestCase):
    ...
    def test_empty_and_first_packet_publish_protocol_correct_state(self) -> None:
        for packet in (UDP_ANALYSIS, TCP_ANALYSIS, packet_at(0, True)):
            ...
            for accumulator in state_components(state):
                if (accumulator is not state.dns_transaction_statistics and accumulator is not state.dns_query_name_statistics
                        and accumulator is not state.dns_resource_record_statistics
                        and accumulator is not state.dns_message_flag_statistics
            ...
            self.assertEqual(state.dns_query_name_statistics.query_name_count, 0)
            self.assertEqual(state.dns_resource_record_statistics.resource_record_count, 0)
            self.assertEqual(state.dns_message_flag_statistics.message_count, 0)
    ...
    def test_frozen_state_retains_sources_and_validates_types_and_identities(self) -> None:
        ...
        self.assertEqual(tuple(field.name for field in fields(state)), names + ("ldap_statistics", "tcp_stream_state", "ldap_stream_state", "ldap_correlation_state", "dns_correlation_state", "dns_transaction_statistics", "dns_query_name_statistics", "dns_resource_record_statistics", "dns_message_flag_statistics", "dns_edns_statistics"))
        self.assertEqual(tuple(vars(state)), names + ("ldap_statistics", "tcp_stream_state", "ldap_stream_state", "ldap_correlation_state", "dns_correlation_state", "dns_transaction_statistics", "dns_query_name_statistics", "dns_resource_record_statistics", "dns_message_flag_statistics", "dns_edns_statistics"))
