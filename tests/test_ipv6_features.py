...
class IPv6FeatureParityTests(unittest.TestCase):
    ...
    def assert_family_equivalence(self, protocol):
        ...
        for field in fields(v4_snapshot.coordinated_state):
            if field.name in ("tcp_stream_state", "tls_record_state", "tls_handshake_state", "tls_client_hello_statistics"):
                continue
    ...
    def test_all_feature_and_state_models_remain_immutable(self):
        for protocol in (6, 17):
            ...
            models += tuple(getattr(state, field.name) for field in fields(state) if field.name not in ("tls_client_hellos", "tls_client_hello_statistics"))
