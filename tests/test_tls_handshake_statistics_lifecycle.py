...
class TLSHandshakeStatisticsLifecycleTests(unittest.TestCase):
    ...
    def test_coordinator_statistic_type_and_legacy_default(self):
        ...
        legacy = tuple(getattr(state, field.name) for field in fields(state) if field.name not in ('tls_handshake_statistics', 'tls_client_hellos', 'tls_client_hello_statistics', 'tls_server_hellos', 'tls_server_hello_statistics', 'ipv6_extension_header_statistics', 'tcp_option_statistics', 'ip_hop_limit_statistics'))
