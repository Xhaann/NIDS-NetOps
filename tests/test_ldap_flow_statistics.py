...
class LDAPFlowTests(unittest.TestCase):
    ...
    def test_statistics_contract_validation_and_legacy_constructor_compatibility(self):
        ...
        legacy = type(state)(*(getattr(state, field.name) for field in fields(state) if field.name not in ('ldap_statistics', 'tcp_stream_state', 'ldap_stream_state', 'ldap_correlation_state')))
