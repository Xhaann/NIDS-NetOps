...
class DNSResourceRecordLifecycleTests(unittest.TestCase):
    ...
    def test_legacy_coordinator_constructor_and_new_field_validation(self):
        ...
        arguments = tuple(getattr(state, field.name) for field in fields(state) if field.name not in ('dns_resource_record_statistics', 'dns_message_flag_statistics', 'dns_edns_statistics', 'dns_stream_state', 'tls_record_state', 'tls_handshake_state', 'tls_handshake_statistics', 'tls_client_hellos'))
