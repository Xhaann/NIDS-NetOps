...
class TLSRecordLifecycleTests(unittest.TestCase):
    ...
    def test_coordinator_type_reference_and_legacy_default(self):
        ...
        legacy = tuple(getattr(state, field.name) for field in fields(state) if field.name not in ('tls_record_state', 'tls_handshake_state', 'tls_handshake_statistics', 'tls_client_hellos'))
