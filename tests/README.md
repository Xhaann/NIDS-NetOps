...
| Research example boundary | [Research examples](test_research_example.py), [observation association](test_research_observation.py), and [timestamp ownership](test_research_observation_timestamps.py): projection/truth retention, optional closed-window context, nested timestamp validation, immutable ownership, validation, equality/repeats, outcome-based PCAP regression, and complete-output equivalence across hash seeds/timezones with detector/application imports blocked. |
