...
def flow_finding_with_address_type(finding, address_type, name="source_address"):
    ...
    changes = {field.name: replace(getattr(state, field.name), identity=identity)
               ...
               and field.name not in ('dns_transaction_statistics', 'dns_query_name_statistics', 'dns_resource_record_statistics', 'dns_message_flag_statistics', 'dns_edns_statistics', 'tls_handshake_statistics', 'tls_client_hellos', 'tls_client_hello_statistics', 'tls_server_hellos')}
