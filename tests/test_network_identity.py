...
def synthetic_window(identity):
    ...
    for field in fields(state):
        if field.name in ("dns_transaction_statistics", "dns_query_name_statistics", "dns_resource_record_statistics", "dns_message_flag_statistics", "dns_edns_statistics", "tls_handshake_statistics", "tls_client_hellos", "tls_client_hello_statistics", "tls_server_hellos"):
            continue
        component = getattr(state, field.name)
        components[field.name] = (
            None if component is None or field.name in ("tcp_stream_state", "tls_record_state", "tls_handshake_state", "tls_client_hello_statistics", "tls_server_hellos") else replace(component, identity=identity)
        )
