...
class DNSQueryNameLifecycleTests(unittest.TestCase):
    ...
    def test_legacy_constructor_and_new_field_validation(self):
        ...
        previous = tuple(getattr(state, field.name) for field in fields(state) if field.name not in ('dns_query_name_statistics', 'dns_resource_record_statistics', 'dns_message_flag_statistics', 'dns_edns_statistics', 'dns_stream_state'))
